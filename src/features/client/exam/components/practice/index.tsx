...
const ExamPractice = ({ examId,part,duration }: IExamStart) => {
  ...
  const getDuration = () => {
    ...
    const total = parseInt(duration ?? "0") * 60 * 1000;
    const time = parseInt(practiceDuration.current ?? "0");
    ...
    <Box mt={"10px"} w={"100%"}>
      ...
      <Flex>
            ...
            <TabPanels>
              {exam?.parts.map((item) => {
                return item.has_group_question ? (
                  <TabPanel key={"part_" + item.id} p={'0px'}>
        ...
        >
          ...
          <Skeleton h={"30px"} isLoaded={duration ? true : false}>
            <Countdown
              date={setTime(duration as string)}
              renderer={rendererCountdown}
              onTick={(e) => (practiceDuration.current = e.total.toString())}
            />
          </Skeleton>
