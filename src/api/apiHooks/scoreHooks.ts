...
export const useGetScoreDetail = (id: string) => {
  return useGetOne<Score>([QueryKeys.GET_SCORE, id], `/admin/scores/${id}`);
};

export const useCreateNewScore = () => {
  return useCreate<FormParams, any>(`/admin/scores`);
};

export const useDeleteScore = () => {
  return useCreate<FormParams, any>(`/admin/scores/delete`);
};
